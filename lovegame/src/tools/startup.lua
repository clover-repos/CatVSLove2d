...
function makeVars()
    ...
    selectSE = love.audio.newSource('res/audio/sfx/select.ogg','static')
...
function startUp()
    ...
    --titlescreen theme
    t = love.audio.newSource("res/audio/music/title.mp3", "stream")
    t:play()
    if devmode == true then
        t:setVolume(0)
    end
    ...
    setScale()
    font = love.graphics.newFont("res/font/font.ttf", 42*scalew)
end
...
function loadNewMap(isD,mapPath, mapID, music,first,t,t2,t3,t4)
    ...
    if level.layers and level.layers["ch"] then
        for i, obj in ipairs(level.layers["ch"].objects) do
            ch = world:newRectangleCollider(obj.x, obj.y, 63, 47)
...
function beforePlayer(image,oY,x,y,scale,coll)
    ...
    local dr = image
      ...
      if coll:getY() + oY <= pl.coll:getY() then
        love.graphics.draw(dr, coll:getX() - x, coll:getY() - y, nil, scale)
      end
    end
end
...
function drawSubWin(x,y,width,height,curve,op,linewidth,r,g,b,r2,g2,b2)
    ...
    love.graphics.setColor(1 - r2, 1 - g2, 1 - b2, 0.8 - op)
