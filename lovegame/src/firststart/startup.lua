...
function loadNewMap(isD,mapPath, mapID, music,first,t,t2,t3,t4)
    ...
    if level.layers and level.layers["ene"] then
        for i, obj in ipairs(level.layers["ene"].objects) do
            enem = world:newBSGRectangleCollider(obj.x, obj.y, obj.width, obj.height,15)
...
function drawSubWin(x,y,width,height,curve)
    local i = w / 10
    if x == nil then x = i end
    if curve == nil then curve = 60 end
...
function spawnSlime(x,y,health)
